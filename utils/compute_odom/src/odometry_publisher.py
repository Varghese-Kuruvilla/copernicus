...
import rospy
import rospkg
from copernicus_msgs.msg import RPM
...
class Odompublisher():
    ...
    def get_params(self):
        with open(self.rospack.get_path('copernicus_control') + "/config/control.yaml",'r') as stream:
            # data = yaml.load(stream, Loader=yaml.FullLoader)
            data = yaml.load(stream)

            self.wheel_radius = data['copernicus_base_controller']['wheel_radius'] 
            # * \
            # data['copernicus_base_controller']['left_wheel_radius_multiplier']
            self.wheel_seperation = data['copernicus_base_controller']['wheel_separation'] 
            # * \
            # data['copernicus_base_controller']['wheel_separation_multiplier']
            self.odomframe = data['copernicus_base_controller']['odom_frame_id']
            self.childframe = data['copernicus_base_controller']['base_frame_id']
